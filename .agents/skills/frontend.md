...
} from "@modules/patient/patient.schema";
...
export function PatientForm({
  ...
  const { form, onSubmit, isEditMode } = usePatientForm({
    mode,
    initialData,
    onSuccess,
  });
...
export const preventSubmitOnEnter: KeyboardEventHandler<HTMLFormElement> = (
  e,
) => {
  if (
    e.key === "Enter" &&
...
<input type="text" readOnly className="caret-transparent" />
