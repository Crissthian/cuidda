 ...
 */
export default function ReferenciaTab({ codigoAtencion, estado = '1', readOnly = false }: ReferenciaTabProps) {
	const [numeroReferencia] = useState(`0000${codigoAtencion?.slice(-4) || '0042'}`)
	...
	const [tratamientos, setTratamientos] = useState<Tratamiento[]>([
		{ id: 1, producto: 'ASPIRINA 100MG', viaAplicacion: '001', comentarios: 'DOSIS DE CARGA ADMINISTRADA' },
		{ id: 2, producto: 'CLOPIDOGREL 75MG', viaAplicacion: '001', comentarios: '300MG VÍA ORAL' }
	])
	...
	const actualizarTratamiento = (index: number, campo: keyof Tratamiento, valor: string) => {
		const nuevosTratamientos = [...tratamientos]
		nuevosTratamientos[index] = { ...nuevosTratamientos[index], [campo]: valor }
		setTratamientos(nuevosTratamientos)
	}
			...
			<fieldset disabled={readOnly} className='space-y-6'>
				...
				<div className='grid grid-cols-4 gap-8'>
					<div className='space-y-2'>
						<label className='block text-sm font-medium text-brand '>N° de referencia</label>
						<input
					...
					<div className='space-y-2'>
						<label className='block text-sm font-medium text-brand'>Accidente</label>
				...
				{/* Datos del destino */}
				<section>
					<h3 className='text-lg font-semibold text-brand mb-4'>Datos del destino</h3>
					<div className='grid grid-cols-4 gap-6'>
						<div className='space-y-2'>
							<label className='block text-sm font-medium text-brand'>Departamento</label>
							<select
						...
						<div className='space-y-2'>
							<label className='block text-sm font-medium text-brand'>Provincia</label>
							<select
								name='ref_provincia'
								value={provincia}
						...
						<div className='space-y-2'>
							<label className='block text-sm font-medium text-brand'>Distrito</label>
							<select
								name='ref_distrito'
								value={distrito}
						...
						<div className='space-y-2'>
							<label className='block text-sm font-medium text-brand'>IPRESS</label>
							<input
								type='text'
								name='ref_ipress'
								value={ipress}
				...
				{/* Enfermedad actual */}
				<section>
					<h3 className='text-lg font-semibold text-brand mb-4'>Enfermedad actual</h3>
					<textarea
				...
				<section>
					<h3 className='text-lg font-semibold text-brand mb-4'>Tratamientos administrados</h3>
					<div className='rounded-lg overflow-hidden'>
						...
						<div className='space-y-2 py-2'>
							{tratamientos.map((tratamiento, index) => (
								...
								>
									<div className='text-sm text-text-primary font-medium'>{index + 1}.-</div>

									<input
										type='text'
										name={`ref_tratamientos[${index}][producto]`}
										value={tratamiento.producto}
										onChange={(e) => actualizarTratamiento(index, 'producto', e.target.value)}
									...
									<select
										...
										onChange={(e) => actualizarTratamiento(index, 'viaAplicacion', e.target.value)}
									...
									<input
										...
										onChange={(e) => actualizarTratamiento(index, 'comentarios', e.target.value)}
				...
				<section>
					<h3 className='text-lg font-semibold text-brand mb-4'>Motivo de referencia</h3>
					<textarea
