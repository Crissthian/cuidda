 ...
 */
export const handlePosicionAnormalDentariaClick = (
  ...
  const seleccion =
    SIGLA_BY_TYPE[selectionMode.type as keyof typeof SIGLA_BY_TYPE];
  if (!seleccion) return false;

  if (isToothInvalid(id, hallazgos)) {
    toast.error(
      "No se puede registrar una posicion anormal dentaria en un diente invalido o ausente.",
    );
    return true;
  }

  const existing = hallazgos.find(
    (h) => h.diente === id && h.hallazgo === "POSICION_ANORMAL_DENTARIA",
  );
  const existingSigla = (
    existing?.sigla ||
    existing?.siglas ||
