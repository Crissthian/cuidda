...
export const handlePiezaSupernumerariaClick: ToothClickHandler = (ctx) => {
  ...
  if (selectionMode.startId === null) {
    if (isToothInvalid(id, hallazgos)) {
      toast.error(
        "No se puede registrar pieza supernumeraria referencia a un diente inválido.",
      );
      return true;
    }
  ...
  if (isUpperArch(startId) !== isUpperArch(endId)) {
    toast.error(
      "La pieza supernumeraria debe registrarse entre dientes del mismo arco.",
    );
    return true;
  }
  ...
  if (!areTeethAdjacent(startId, endId, upperTeeth, lowerTeeth)) {
    toast.error(
      "La pieza supernumeraria debe referenciarse entre dientes contiguos.",
    );
    return true;
  }
  ...
  const exists = piezasSupernumerarias?.some(
    (p) =>
      (p.startId === startId && p.endId === endId) ||
      (p.startId === endId && p.endId === startId),
  );

  if (exists) {
    toast.error(
      "Ya existe una pieza supernumeraria registrada en esta ubicación.",
    );
    setSelectionMode((prev) => ({ ...prev, startId: null }));
    return true;
  }
