...
export function handleEctopicaClick(ctx: ClickHandlerContext): boolean {
	...
	if (isToothInvalid(id, hallazgos)) {
		toast.error('No se puede registrar pieza ectópica en un diente ausente o inválido.')
		return true
	}

	// 2. Buscar si ya existe el hallazgo en este diente
	const existingHallazgo = hallazgos.find((h) => h.diente === id && h.hallazgo === 'PIEZA_ECTOPICA')
