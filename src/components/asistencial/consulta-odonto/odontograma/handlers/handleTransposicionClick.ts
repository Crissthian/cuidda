...
export const handleTransposicionClick = (ctx: ClickHandlerContext): boolean => {
	const { id, selectionMode, setSelectionMode, hallazgos, onHallazgoCreated, onHallazgoDeleted } = ctx
	...
	const existing = hallazgos.find(
		(h) =>
			...
			((h.diente === startId && h.dienteFinal === endId) || (h.diente === endId && h.dienteFinal === startId))
	)
