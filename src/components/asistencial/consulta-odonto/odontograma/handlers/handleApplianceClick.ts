...
export const handleApplianceClick: ToothClickHandler = (ctx) => {
  ...
  if (selectionMode.type === "fixed") {
    ...
    const hasOverlap = appliances.some(
      (a) =>
        a.type === "fixed" &&
        getTeethInRange(a.startId, a.endId, upperTeeth, lowerTeeth).some((t) =>
          range.includes(t),
        ),
    );
    if (hasOverlap) {
    ...
    if (
      range.some((tid) =>
        hasRemovableApplianceCovering(tid, appliances, upperTeeth, lowerTeeth),
      )
    ) {
      toast.error(
        "No puede coexistir aparato fijo y removible en el mismo diente.",
      );
      return true;
    }
  }
  ...
  if (selectionMode.type === "removable") {
    if (hasProtesisTotal(startIsUpper ? "upper" : "lower", hallazgos)) {
      toast.error(
        "No se puede agregar aparato removible en arcada con prótesis total.",
      );
      return true;
    }
    const hasOverlap = appliances.some(
    ...
    if (hasOverlap) {
      toast.error(
        "Ya existe un aparato removible sobre alguno de estos dientes.",
      );
      return true;
    }
    if (
      range.some((tid) =>
        hasFixedApplianceCovering(tid, appliances, upperTeeth, lowerTeeth),
      )
    ) {
      toast.error(
        "No puede coexistir aparato fijo y removible en el mismo diente.",
