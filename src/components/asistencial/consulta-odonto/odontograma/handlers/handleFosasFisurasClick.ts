...
export const handleFosasFisurasClick = (ctx: ClickHandlerContext): boolean => {
  ...
  if (
    ...
    selectionMode.type !== "fosas_fisuras_profundas"
  ) {
    return false;
  }
  ...
  if (isToothInvalid(id, hallazgos)) {
    toast.error(
      'No se puede registrar "Fosas y Fisuras Profundas" en un diente inválido.',
    );
    return true;
  }
  ...
  const existing = hallazgos.find(
    (h) => h.hallazgo === "FOSAS_FISURAS_PROFUNDAS" && h.diente === id,
  );
