...
export function handleMovilidadClick(ctx: ClickHandlerContext): boolean {
  ...
  if (isToothInvalid(id, hallazgos)) {
    toast.error(
      "No se puede registrar movilidad en un diente ausente o inválido.",
    );
    return true;
  }
  ...
  const existingHallazgo = hallazgos.find(
    (h) => h.diente === id && h.hallazgo === "MOVILIDAD_PATOLOGICA",
  );
  ...
  if (existingHallazgo) {
    ...
    const existingGrado = parseInt(
      existingHallazgo.siglas?.replace("M", "") || "0",
    );
