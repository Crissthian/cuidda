...
export const handleCrownClick: ToothClickHandler = (ctx) => {
  const { id, hallazgos, selectionMode, onHallazgoCreated, crownLabelInput } =
    ctx;

  if (
    selectionMode.type !== "crown" &&
    selectionMode.type !== "corona_temporal"
  )
    return false;

  if (isToothInvalid(id, hallazgos)) {
    toast.error(
      "No se puede colocar corona en un diente ausente, extraído, no erupcionado o agenésico.",
    );
    return true;
  }
