...
export const handleFractureClick: ToothClickHandler = (ctx) => {
	...
	if (selectionMode.type !== 'fracture_crown' && selectionMode.type !== 'fracture_root') {
		return false
	}
	...
	if (existing) {
		toast.error(`Ya existe una fractura de ${type === 'crown' ? 'corona' : 'raíz'} en este diente.`)
		return true
	}

	const hallazgoName = selectionMode.type === 'fracture_crown' ? 'FRACTURA_CORONA' : 'FRACTURA_RAIZ'
	const spec = selectionMode.type === 'fracture_crown' ? 'Fractura Corona' : 'Fractura Raíz'
