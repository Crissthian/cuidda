 ...
 */
export const handleTratamientoConductoClick = (ctx: ClickHandlerContext, subType: 'TC' | 'PC'): boolean => {
	const { id, selectionMode, hallazgos, onHallazgoCreated, onHallazgoDeleted } = ctx

	if (
		!selectionMode.isActive ||
		(selectionMode.type !== 'tratamiento_conducto' && selectionMode.type !== 'pulpectomia')
	) {
		return false
	}

	// Buscar hallazgo existente de tratamiento de conducto para este diente
	const existing = hallazgos.find((h) => h.hallazgo === 'TRATAMIENTO_CONDUCTO' && h.diente === id)
	...
	if (existing) {
		...
		const newHallazgo: HallazgoClinico = {
			...
			especificacion: subType === 'TC' ? 'Tratamiento de Conducto' : 'Pulpectomía',
