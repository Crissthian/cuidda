 ...
 */
export const handleRemanenteRadicularClick = (ctx: ClickHandlerContext): boolean => {
	const { id, selectionMode, onHallazgoCreated, onHallazgoDeleted, hallazgos, viewMode } = ctx
	...
	if (isToothInvalid(id, hallazgos)) {
		toast.error('No se puede registrar Remanente Radicular en un diente ausente.')
		return true
	}

	// Buscar si ya existe remanente radicular en el diente
	const existing = hallazgos.find((h) => h.hallazgo === 'REMANENTE_RADICULAR' && h.diente === id)
