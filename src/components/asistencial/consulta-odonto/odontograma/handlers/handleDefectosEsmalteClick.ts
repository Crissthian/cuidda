...
export const handleDefectosEsmalteClick: ToothClickHandler = (ctx) => {
	...
	if (isToothInvalid(id, hallazgos)) {
		toast.error('No se puede registrar este hallazgo en un diente ausente o inválido.')
		return true
	}
	...
	const exists = hallazgos.some((h) => h.diente === id && h.hallazgo === 'HIPOPLASIA_ESMALTE' && h.sigla === sigla)
