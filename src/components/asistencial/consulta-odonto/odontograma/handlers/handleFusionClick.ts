...
export const handleFusionClick: ToothClickHandler = (ctx) => {
	const { id, hallazgos, selectionMode, setSelectionMode, onHallazgoCreated, fusions, upperTeeth, lowerTeeth } = ctx
	...
	const existing = fusions.find(
		(f) => (f.startId === startId && f.endId === endId) || (f.startId === endId && f.endId === startId)
	)
