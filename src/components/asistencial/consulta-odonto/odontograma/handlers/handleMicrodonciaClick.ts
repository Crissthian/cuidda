...
/**
 * Handler para el hallazgo: MICRODONCIA
 * @param ctx Contexto con toda la información necesaria para los handlers
 * @returns true si se manejó el click, false si no
 */
export function handleMicrodonciaClick(ctx: ClickHandlerContext): boolean {
	const { id, hallazgos, selectionMode, onHallazgoCreated, onHallazgoDeleted } = ctx

	if (!selectionMode.isActive || selectionMode.type !== 'microdoncia') {
		return false
	}

	// 1. Validaciones básicas
	if (isToothInvalid(id, hallazgos)) {
		toast.error('No se puede registrar microdoncia en un diente ausente o inválido.')
		return true
	}

	// 2. Buscar si ya existe el hallazgo en este diente
	const existingHallazgo = hallazgos.find((h) => h.diente === id && h.hallazgo === 'MICRODONCIA')
