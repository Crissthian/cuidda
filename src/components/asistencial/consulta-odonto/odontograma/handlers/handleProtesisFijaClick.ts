...
import { isToothInvalid, isUpperArch, getTeethInRange, hasProtesisTotal } from '../utils'
...
export const handleProtesisFijaClick: ToothClickHandler = (ctx) => {
	const { id, hallazgos, selectionMode, setSelectionMode, onHallazgoCreated, upperTeeth, lowerTeeth } = ctx
	...
	if (hasProtesisTotal(isUpper ? 'upper' : 'lower', hallazgos)) {
		toast.error('No se puede agregar prótesis dental en arcada con prótesis total.')
		return true
	}
	...
	if (range.length < 2) {
		toast.error('La prótesis fija requiere al menos 2 dientes (pilar inicial y final).')
		return true
	}
