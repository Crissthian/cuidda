...
import type { SelectionMode, SelectionType, StatusType, HallazgoClinico } from '../types'
...
export function useSelectionMode({
	...
	const [internalSelectionMode, internalSetSelectionMode] = useState<SelectionMode>(INITIAL_SELECTION_MODE)
	...
	const startSelection = useCallback(
		(type: SelectionType, status: StatusType) => {
			...
			if (type === 'edentulous_total') {
				...
				const existing = hallazgos.find((h) => h.hallazgo === 'EDENTULO_TOTAL' && h.especificacion === arch)
