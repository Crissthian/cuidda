 ...
 */
export function useHallazgosDerivados(
  ...
  const appliances = useMemo<Appliance[]>(
    () =>
      hallazgos
        .filter(
          (h) =>
            h.hallazgo === "APARATO_ORTODONTICO_FIJO" ||
            h.hallazgo === "APARATO_REMOVIBLE",
        )
        .map((h) => ({
          id: h.id,
          startId: h.diente!,
          ...
          type:
            h.hallazgo === "APARATO_ORTODONTICO_FIJO" ? "fixed" : "removable",
  ...
  const crowns = useMemo<Crown[]>(
    () =>
      hallazgos
        .filter(
          (h) => h.hallazgo === "CORONA" || h.hallazgo === "CORONA_TEMPORAL",
        )
        .map((h) => ({
          id: h.id,
          toothId: h.diente!,
          ...
          status:
            h.hallazgo === "CORONA_TEMPORAL"
              ? "malo"
              : (h.estado as "bueno" | "malo"),
  ...
  const fractures = useMemo<Fracture[]>(
    () =>
      hallazgos
        .filter(
          (h) =>
            h.hallazgo === "FRACTURA_CORONA" || h.hallazgo === "FRACTURA_RAIZ",
        )
        .map((h) => ({
          id: h.id,
          toothId: h.diente!,
    ...
    () =>
      hallazgos
        ...
        .map((h) => ({
          id: h.id,
          toothId: h.diente!,
          direction: h.especificacion?.toLowerCase().includes("mesial")
            ? "mesial"
            : "distal",
          status: h.estado as "bueno" | "malo",
        })),
