 ...
 */
export const isToothInCurrentView = (
  id: number,
  upperTeeth: ToothData[],
  lowerTeeth: ToothData[],
): boolean => {
...
export const isToothInvalid = (
  ...
  return hallazgos.some(
    (h) => h.diente === toothId && invalidStates.includes(h.hallazgo),
  );
};
...
export const hasCrown = (
  ...
  return hallazgos.some(
    (h) =>
      h.diente === toothId &&
      (h.hallazgo === "CORONA" || h.hallazgo === "CORONA_TEMPORAL"),
  );
};
...
export const hasProtesisTotal = (
  ...
  return hallazgos.some(
    (h) =>
      ...
      h.especificacion
        ?.toLowerCase()
        .includes(arch === "upper" ? "superior" : "inferior"),
  );
};
 ...
 */
export const hasDiastemaBetween = (
  id1: number,
  id2: number,
  diastemas: Diastema[],
): boolean => {
  return diastemas.some(
    (d) =>
      (d.startId === id1 && d.endId === id2) ||
      (d.startId === id2 && d.endId === id1),
  );
};
