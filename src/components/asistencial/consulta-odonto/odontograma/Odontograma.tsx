...
import type { OdontogramaProps, HallazgoClinico, SelectionMode, ToothData } from './types'
import { ADULT_TEETH_UPPER, ADULT_TEETH_LOWER, CHILD_TEETH_UPPER, CHILD_TEETH_LOWER } from './constants'
import { ToothColumn, OdontogramaToolbar, OdontogramaSvgOverlay } from './components'
import { getTeethInRange, isToothInvalid, hasBridgeBetween, isUpperArch } from './utils'
...
export const Odontograma: React.FC<OdontogramaProps> = ({ hallazgos = [], onHallazgoCreated, onHallazgoDeleted }) => {
	// ============================================================================
	...
	useEffect(() => {
		...
		appliances
			...
			.forEach((app) => {
				if (!allCurrentViewIds.includes(app.startId) && !allCurrentViewIds.includes(app.endId)) return

				const range = getTeethInRange(app.startId, app.endId, upperTeeth, lowerTeeth)
				const validTeeth = range.filter((id) => !isToothInvalid(id, hallazgos))
				// Si faltan dientes intermedios o extremos, eliminar aparato
				if (validTeeth.length < 2 || range.some((id) => isToothInvalid(id, hallazgos))) {
		...
		// 5. Validar Diastemas
		diastemas.forEach((d) => {
			if (!allCurrentViewIds.includes(d.startId) && !allCurrentViewIds.includes(d.endId)) return
			if (isToothInvalid(d.startId, hallazgos) || isToothInvalid(d.endId, hallazgos)) {
				onHallazgoDeleted(d.id)
				return
			}
			// Eliminar diastema si hay un puente encima
			if (hasBridgeBetween(d.startId, d.endId, appliances, upperTeeth, lowerTeeth)) {
				onHallazgoDeleted(d.id)
			}
		})
		...
		fusions.forEach((f) => {
			if (!allCurrentViewIds.includes(f.startId) && !allCurrentViewIds.includes(f.endId)) return
			if (isToothInvalid(f.startId, hallazgos) || isToothInvalid(f.endId, hallazgos)) onHallazgoDeleted(f.id)
		})
		...
		piezasSupernumerarias.forEach((p) => {
			if (!allCurrentViewIds.includes(p.startId) && !allCurrentViewIds.includes(p.endId)) return
			if (isToothInvalid(p.startId, hallazgos) || isToothInvalid(p.endId, hallazgos)) onHallazgoDeleted(p.id)
		})
		...
		restauraciones.forEach((r) => {
			...
			if (isToothInvalid(r.toothId, hallazgos) || r.surfaces.length === 0) onHallazgoDeleted(r.id)
		})
		...
		superficiesDesgastadas.forEach((s) => {
			...
			if (isToothInvalid(s.toothId, hallazgos) || s.surfaces.length === 0) onHallazgoDeleted(s.id)
		})
		...
		transposiciones.forEach((t) => {
			if (!allCurrentViewIds.includes(t.startId) && !allCurrentViewIds.includes(t.endId)) return
			if (isToothInvalid(t.startId, hallazgos) || isToothInvalid(t.endId, hallazgos)) onHallazgoDeleted(t.id)
		})
	 ...
	 */
	const createHandlerContext = (toothId: number, positionOverride?: 'upper' | 'lower'): ClickHandlerContext => ({
		id: toothId,
	...
	const handleZoneClick = useCallback(
		(toothId: number, zone: string) => {
			...
			if (selectionMode.isActive && selectionMode.type === 'restauracion_temporal') {
			...
			if (selectionMode.isActive && selectionMode.type === 'tratamiento_conducto') {
			...
			const isCariesMode =
				...
				['caries_mb', 'caries_ce', 'caries_cd', 'caries_cdp'].includes(selectionMode.type)
		...
		[selectionMode, hallazgos, cariesList, onHallazgoCreated, onHallazgoDeleted, createHandlerContext]
	)
	...
	const renderTeethRow = (teeth: ToothData[], position: 'upper' | 'lower', filter: (t: ToothData) => boolean) => {
		return teeth.filter(filter).map((tooth) => (
			<ToothColumn
				...
				restauracionTemporal={restauracionesTemporales.find((r) => r.toothId === tooth.id)}
				superficieDesgastada={superficiesDesgastadas.find((s) => s.toothId === tooth.id)}
				tratamientoConducto={tratamientosConducto.find((t) => t.toothId === tooth.id)}
				...
				posicionAnormalDentaria={posicionesAnormales.find((p) => p.toothId === tooth.id)}
				movilidadPatologica={movilidadPatologica.find((m) => m.toothId === tooth.id)}
				piezaEctopica={piezasEctopicas.find((p) => p.toothId === tooth.id)}
				remanenteRadicular={remanenteRadicular.find((r) => r.toothId === tooth.id)}
							...
							setTimeout(() => {
								domToImage.default
									...
									.then((blob: Blob | null) => {
										...
										if (blob) saveAs(blob, `odontograma_${new Date().getTime()}.png`)
									})
									.catch((error: unknown) => {
			...
			>
				...
				<div className='flex gap-2 mb-4 border border-transparent'>
					...
					<div className='flex gap-1 border border-transparent'>
						{renderTeethRow(upperTeeth, 'upper', (t) => t.id < 20 || (t.id >= 50 && t.id < 60))}
					</div>
				...
				<div className='flex gap-2 mt-4 border border-transparent '>
					...
					<div className='flex gap-1 border border-transparent'>
						{renderTeethRow(lowerTeeth, 'lower', (t) => t.id >= 40 || t.id >= 80)}
					</div>
					{/* Cuadrante Inferior Derecho */}
					<div className='flex gap-1 border border-transparent'>
						{renderTeethRow(lowerTeeth, 'lower', (t) => t.id >= 30 && t.id < 40)}
					</div>
				</div>
			</div>
		</div>
