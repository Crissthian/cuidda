...
import { getToothRect, isUpperTooth, OVERLAY_COLORS, VERTICAL_OFFSETS } from './types'
...
export const EspigoOverlay: React.FC<EspigoOverlayProps> = ({
	...
	return (
		<>
			{espigos.map((esp) => {
				if (!isToothInCurrentView(esp.toothId, upperTeeth, lowerTeeth)) return null

				const data = getToothRect(esp.toothId, toothRefs.current, containerRef.current)
				if (!data) return null

				const { cx, topY } = data
				const isUpper = isUpperTooth(esp.toothId)
				const offsets = isUpper ? VERTICAL_OFFSETS.upper : VERTICAL_OFFSETS.lower
				const crownY = topY + offsets.crownCenter
				const rootY = topY + offsets.rootCenter

				const color = esp.status === 'bueno' ? OVERLAY_COLORS.bueno : OVERLAY_COLORS.malo
				const squareSize = 16
				...
				return (
					<g key={esp.id}>
						...
						<line x1={cx} y1={crownY} x2={cx} y2={rootY} stroke={color} strokeWidth='3' />
