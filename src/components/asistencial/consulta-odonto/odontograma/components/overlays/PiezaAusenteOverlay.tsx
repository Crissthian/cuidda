	...
	return (
		<>
			{piezasAusentes.map((pieza) => {
				if (!isToothInCurrentView(pieza.toothId, upperTeeth, lowerTeeth)) return null

				const data = getToothRect(pieza.toothId, toothRefs.current, containerRef.current)
				if (!data) return null
				...
				return (
					<g key={pieza.id}>
						<line x1={x1} y1={y1} x2={x2} y2={y2} stroke={OVERLAY_COLORS.bueno} strokeWidth='3' />
						<line x1={x2} y1={y1} x2={x1} y2={y2} stroke={OVERLAY_COLORS.bueno} strokeWidth='3' />
					</g>
				)
			})}
		</>
