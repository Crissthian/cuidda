...
import { getCoordinates, generateZigzagPath, isToothInCurrentView } from '../../utils'
	...
	return (
		<>
			{appliances.map((app) => {
				...
				const start = getCoordinates(app.startId, toothRefs.current, containerRef.current)
				const end = getCoordinates(app.endId, toothRefs.current, containerRef.current)
				if (!start || !end) return null

				const color = app.status === 'bueno' ? OVERLAY_COLORS.bueno : OVERLAY_COLORS.malo
