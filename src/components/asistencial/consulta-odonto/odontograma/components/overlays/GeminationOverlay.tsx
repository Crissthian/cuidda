...
export const GeminationOverlay: React.FC<GeminationOverlayProps> = ({
	...
	return (
		<>
			{geminations.map((gem) => {
				if (!isToothInCurrentView(gem.toothId, upperTeeth, lowerTeeth)) return null
				...
				return <circle key={gem.id} cx={cx} cy={cy} r={radius} stroke={color} strokeWidth='2' fill='none' />
			})}
		</>
	)
}
