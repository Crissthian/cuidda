...
import { getToothRect, isUpperTooth, VERTICAL_OFFSETS, OVERLAY_COLORS } from './types'
	...
	return (
		<>
			{giroversions.map((giro) => {
				if (!isToothInCurrentView(giro.toothId, upperTeeth, lowerTeeth)) return null

				const data = getToothRect(giro.toothId, toothRefs.current, containerRef.current)
				if (!data) return null

				const { cx, topY } = data
				const isUpper = isUpperTooth(giro.toothId)
				const cy = topY + (isUpper ? VERTICAL_OFFSETS.upper.crownCenter : VERTICAL_OFFSETS.lower.crownCenter)
