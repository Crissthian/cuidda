      ...
      {recetaPages.map((page, pageIndex) => (
        <Page key={pageIndex} size="A4" style={recetaStyles.page}>
          ...
          <View style={recetaStyles.patientInfo}>
            ...
            >
              <Text>
                <Text style={recetaStyles.infoLabel}>Código: </Text>
            ...
            >
              <Text>
                <Text style={recetaStyles.infoLabel}>C.O.P.: </Text>
            ...
            {formData.diagnosticos && formData.diagnosticos.length > 0 && (
              <View style={recetaStyles.diagnosisRow}>
                <Text style={recetaStyles.infoLabel}>Diagnósticos: </Text>
            ...
            <View style={recetaStyles.medicationContainer}>
              <View style={recetaStyles.medicationColumn}>
                {page.leftColumn.map((item, idx) => (
                  <View key={idx} style={recetaStyles.medicationItem}>
                    <Text
                      style={{
                        fontSize: 8,
                        fontWeight: "bold",
                    ...
                    >
                      {idx * 2 + 1}. {item.des_prod} - Cant: {item.cant}
                    </Text>
                    <Text style={recetaStyles.medicationDetails}>
                      Vía: {item.via_apli} | Frecuencia: {item.frec} | Duración:{" "}
                      {item.durac} días
                    </Text>
                    {item.comen ? (
                      <Text style={recetaStyles.medicationDetails}>
              ...
              <View style={recetaStyles.medicationColumn}>
                {page.rightColumn.map((item, idx) => (
                  <View key={idx} style={recetaStyles.medicationItem}>
                    ...
                    >
                      {idx * 2 + 2}. {item.des_prod} - Cant: {item.cant}
                    </Text>
                    <Text style={recetaStyles.medicationDetails}>
                      Vía: {item.via_apli} | Frecuencia: {item.frec} | Duración:{" "}
                      {item.durac} días
          ...
          <View style={recetaStyles.notesContainer}>
            {formData.alergias ? (
              <View style={recetaStyles.notesBox}>
                <Text style={recetaStyles.notesTitle}>Alergias:</Text>
                <Text style={recetaStyles.notesText}>{formData.alergias}</Text>
              </View>
            ) : null}
            {formData.recomendaciones ? (
              <View style={recetaStyles.notesBox}>
                <Text style={recetaStyles.notesTitle}>Recomendaciones:</Text>
          ...
          <View style={recetaStyles.footer}>
            <Text>Lucemedic Salud Ocupacional - Área de Odontología</Text>
            <Text>
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      const formData: FormData = {
        ...
        recetas,
        datosMedico: {
          nombre_medico: datosEvento?.nombreMedico || "DRA. ODONTÓLOGA",
          cmp: "COP-12845",
        },
        alergias,
        recomendaciones,
